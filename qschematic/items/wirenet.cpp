...
void WireNet::from_container(const gpds::container& container)
{
  ...
  // Wires
  {
    // Fix the bug when the wires container is null
    auto wiresContainer_p = container.get_value<gpds::container*>("wires");

    if (wiresContainer_p == std::nullopt)
    {
      return;
    }

    const gpds::container& wiresContainer = *wiresContainer_p.value();
